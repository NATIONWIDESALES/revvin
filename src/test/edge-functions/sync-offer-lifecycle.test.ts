...
describe("runSyncLifecycle — single business", () => {
  it("pauses the most expensive active offer first when the wallet drops below committed", async () => {
    ...
    // Costs (= payout): 500, 1000, 200 → committed = 1700, available = 300
    state.offers = [
      offer("o-cheap", "biz-1", 200), // cost 200
      offer("o-mid", "biz-1", 500), // cost 500
      offer("o-expensive", "biz-1", 1000), // cost 1000
    ];

    const result = await runSyncLifecycle(makeMock(state), "user-1");

    expect(result.success).toBe(true);
    // Expensive (1000) + mid (500) paused; committed=200 ≤ 300 → stop.
  ...
  it("reactivates auto-paused offers cheapest-first when the wallet is topped up", async () => {
    ...
    state.businessesByUser.set("user-1", [{ id: "biz-1" }]);
    state.offers = [
      offer("o-a", "biz-1", 200, "paused", "low_wallet"), // cost 200
      offer("o-b", "biz-1", 500, "paused", "low_wallet"), // cost 500
      offer("o-c", "biz-1", 1000, "paused", "low_wallet"), // cost 1000 -- can't fit
    ];
    ...
    expect(result.reactivated).toBe(2); // o-a (200) + o-b (500) = 700 ≤ 800
...
describe("runSyncLifecycle — multi-business", () => {
  it("pauses across ALL of the user's businesses (regression for the .maybeSingle bug)", async () => {
    ...
    state.offers = [
      offer("o-A1", "biz-A", 800), // cost 800
      offer("o-B1", "biz-B", 400), // cost 400
      offer("o-B2", "biz-B", 100), // cost 100
    ];

    const result = await runSyncLifecycle(makeMock(state), "user-1");

    // Total committed = 1300, available = 300. Pause expensive→cheap until ≤300:
    // Pause o-A1 (800) → 500 left, still >300 → pause o-B1 (400) → 100 left, ≤300 stop.
    expect(result.paused).toBe(2);
  ...
  it("reactivates across multiple businesses when funded", async () => {
    state.walletAvailable = 700;
    state.businessesByUser.set("user-1", [{ id: "biz-A" }, { id: "biz-B" }]);
    state.offers = [
      offer("o-A1", "biz-A", 200, "paused", "low_wallet"), // 200
      offer("o-B1", "biz-B", 400, "paused", "low_wallet"), // 400
    ];

    const result = await runSyncLifecycle(makeMock(state), "user-1");

    expect(result.reactivated).toBe(2); // 200 + 400 = 600 ≤ 700
  });
});
...
describe("runSyncLifecycle — exact-balance edge cases", () => {
  it("does NOT pause when committed == available (boundary, > vs >=)", async () => {
    state.walletAvailable = 100;
    state.businessesByUser.set("user-1", [{ id: "biz-1" }]);
    state.offers = [offer("o-1", "biz-1", 100)]; // cost 100
    const result = await runSyncLifecycle(makeMock(state), "user-1");
    expect(result.paused).toBe(0);
  });

  it("DOES pause when committed exceeds available by 1 cent", async () => {
    state.walletAvailable = 99.99;
    state.businessesByUser.set("user-1", [{ id: "biz-1" }]);
    state.offers = [offer("o-1", "biz-1", 100)]; // cost 100
    const result = await runSyncLifecycle(makeMock(state), "user-1");
    expect(result.paused).toBe(1);
  ...
  it("reactivates when adding the offer brings committed exactly to available (≤ boundary)", async () => {
    state.walletAvailable = 100;
    state.businessesByUser.set("user-1", [{ id: "biz-1" }]);
    state.offers = [offer("o-1", "biz-1", 100, "paused", "low_wallet")]; // cost 100
    const result = await runSyncLifecycle(makeMock(state), "user-1");
    expect(result.reactivated).toBe(1);
  });
