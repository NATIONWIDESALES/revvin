 ...
 * page so validation, invite handling and tracking never diverge.
 */
const SignupForm = ({
  formId = "signup-form",
  ...
  /** Fires once per mount, on first focus of any field. */
  ...
  return (
    <>
      {inviteCode && <InviteBanner code={inviteCode} compact className="mb-3" />}

      <form id={formId} onSubmit={handleSubmit} onFocus={handleFieldFocus} className="space-y-3">
        <div>
          <Label htmlFor={`${formId}-biz`}>Business name</Label>
          <Input id={`${formId}-biz`} autoComplete="organization" value={businessName} onChange={(e) => setBusinessName(e.target.value)} placeholder="Your business name" className="mt-1.5" required />
        </div>
        <div>
          <Label htmlFor={`${formId}-name`}>Your name</Label>
          <Input id={`${formId}-name`} autoComplete="name" value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="Jane Smith" className="mt-1.5" />
        </div>
        <div>
          <Label htmlFor={`${formId}-email`}>Work email</Label>
          <Input id={`${formId}-email`} type="email" autoComplete="email" inputMode="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@yourbusiness.com" className="mt-1.5" required />
        </div>
        <div>
...
export const SIGNUP_SUBHEAD = (inviteCode: string | null) =>
  inviteCode
