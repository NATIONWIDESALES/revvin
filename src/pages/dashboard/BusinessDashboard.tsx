...
import {
  ...
  Wallet, ShieldCheck, ArrowUpRight, ArrowDownLeft, RefreshCw
} from "lucide-react";
...
const BusinessDashboard = () => {
  ...
  const handleAccept = async (ref: any) => {
    ...
    if (payoutAmt > 0) {
      reserveFunds(payoutAmt, ref.id, `Reserved $${payoutAmt} for ${ref.customer_name}`);
    }
  ...
  const handleWon = async (ref: any) => {
    ...
    if (payoutAmt > 0) {
      releasePayout(payoutAmt, ref.id, `Payout released: $${referrerPayout} to referrer, $${payoutAmt - referrerPayout} platform fee`);
    }
  ...
  const handleLost = async (ref: any) => {
    const payoutAmt = ref.offers?.payout_type === "flat" ? Number(ref.offers.payout) : 0;
    if (ref.status === "accepted" && payoutAmt > 0) {
    ...
    toast({ title: "Marked as lost", description: ref.status === "accepted" ? "Escrowed funds returned to available balance." : undefined });
  };
        ...
        <motion.div initial="hidden" animate="visible">
          ...
          <motion.div variants={fadeUp} custom={0.5} className="mb-8 rounded-2xl border-2 border-primary/20 bg-card p-6 shadow-sm">
            <div className="flex items-center justify-between mb-5">
              <h2 className="font-display text-lg font-bold flex items-center gap-2">
                <Wallet className="h-5 w-5 text-primary" /> Revvin Wallet
              </h2>
              <div className="flex gap-2">
                {[250, 500, 1000, 2500].map((amt) => (
                  <Button key={amt} size="sm" variant="outline" className="text-xs" onClick={() => { addFunds(amt); toast({ title: "Funds added", description: `$${amt} added to your wallet.` }); }}>
                    +${amt}
                  </Button>
                ))}
              </div>
            ...
            <div className="grid gap-4 sm:grid-cols-3 mb-5">
              <div className="rounded-xl border border-earnings/20 bg-earnings/5 p-4 text-center">
                ...
                <p className="font-display text-3xl font-bold text-earnings">${wallet.available.toLocaleString()}</p>
              </div>
              <div className="rounded-xl border border-primary/20 bg-primary/5 p-4 text-center">
                <p className="text-xs text-muted-foreground font-medium mb-1">Reserved (Escrow)</p>
                <p className="font-display text-3xl font-bold text-primary">${wallet.reserved.toLocaleString()}</p>
              </div>
              <div className="rounded-xl border border-border bg-muted/30 p-4 text-center">
                <p className="text-xs text-muted-foreground font-medium mb-1">Total Funded</p>
                <p className="font-display text-3xl font-bold text-foreground">${wallet.totalFunded.toLocaleString()}</p>
              </div>
            </div>
            {/* Transaction Log */}
            <div>
                ...
                {wallet.transactions.slice(0, 8).map((tx) => {
                  ...
                  return (
                    <div key={tx.id} className="flex items-center justify-between rounded-lg bg-muted/30 px-3 py-2 text-xs">
                      ...
                      <span className={`font-mono font-medium ${cfg.color}`}>{cfg.sign}${tx.amount}</span>
                    </div>
                  );
                })}
              </div>
          ...
          <motion.div variants={fadeUp} custom={1} className="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {[
              ...
              { label: "Cost per Close", value: wonCount > 0 ? `$${avgAcquisitionCost}` : "—", icon: Target, color: "text-primary", bgColor: "bg-primary/10" },
              { label: "Total Paid Out", value: `$${totalPaid.toLocaleString()}`, icon: DollarSign, color: "text-earnings", bgColor: "bg-earnings/10" },
              ...
              <div key={s.label} className="rounded-2xl border border-border bg-card p-5 shadow-sm">
                <div className="flex items-center gap-3">
                  <div className={`rounded-xl p-2.5 ${s.bgColor}`}><s.icon className={`h-5 w-5 ${s.color}`} /></div>
                  <div>
                  ...
                  return (
                    <div key={offer.id} className="rounded-2xl border border-border bg-card p-5 shadow-sm">
                      ...
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <div className="text-xs text-muted-foreground">{offerRefs.length} refs • {offerWon} won</div>
                      </div>
          ...
          {/* Referrals Inbox — with escrow actions */}
